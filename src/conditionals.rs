...
#[derive(PartialEq, Debug)]
// Declare an enum for Car transmission type
enum Transmission {
    Manual,
    SemiAuto,
    Automatic,
...
// - miles (u32)
// Return tuple with car age ("New" or "Used") and milleage
...
fn car_factory(color: String, motor: Transmission, roof: bool, miles: u32) -> Car {
    ...
    if car_quality(miles).0 == Age::Used {
        ...
        if roof {
            println!(
                "Preparing a New car: {:?}, {}, Hard top, {} miles",
                motor, color, miles,
            );
        } else {
            println!(
                "Preparing a New car: {:?}, {}, Convertible, {} miles",
                motor, color, miles
            );
        }
    }
    // Create a new "Car" intance as requested
...
pub fn run() {
    ...
    // Car order #2: Used, SemiAuto, Convertible
